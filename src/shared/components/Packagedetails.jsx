...
//         setFormData(prev => ({ ...prev, package: details.packageData.title })); // prefill formData
...
//             <div className="max-w-[566px] relative shadow-[0_0_10px_rgba(0,0,0,0.15)] rounded-[30px]">
//               <img
...
//                 <div>
//                   <label htmlFor="package" className="block text-sm poppins-semibold text-gray-700 mb-2">
//                     Package *
//                   </label>
//                   <input
//                     type="text"
...
function Packagedetails() {
  ...
  const navigate = useNavigate();
  const [packageDetails, setPackageDetails] = useState(null);
  const [selectedCars, setSelectedCars] = useState({}); // Object to store selected car for each section
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
    ...
    try {
      ...
      if (details) {
        ...
        Object.keys(details).forEach(key => {
          if (key.startsWith('carOptions') || key === 'carOptions') {
            const firstCar = Object.values(details[key])[0];
            initialSelectedCars[key] = firstCar;
          }
        });
  ...
  const handleCarSelect = (car, sectionKey) => {
  ...
  const getCarBookingSections = () => {
    ...
    if (packageDetails.carOptions && packageDetails.carBookingSection) {
      sections.push({
        key: 'carOptions',
        title: packageDetails.carBookingSection.title,
        cars: packageDetails.carOptions
      });
    }

    // Check for additional car sections (carOptions2, carOptions3, etc.)
    let sectionIndex = 2;
  ...
  const renderCarBookingSection = (section, index) => {
    ...
    return (
      <div key={section.key} className="mt-16">
        <h2 className="md:text-2xl text-xl text-[#6CBF02] poppins-semibold text-center mb-10">
          {section.title}
        </h2>
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="w-full space-y-5">
            {Object.values(section.cars).map((car) => (
              <div
                key={car.id}
  ...
  const carBookingSections = getCarBookingSections();
